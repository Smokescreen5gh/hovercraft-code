 ...
 // We want period copy to be greater than 0 because we cant divide by 0 
  if (pulseAvailableCopy && periodCopy > 0) {
    ...
    if (acceptPulse) {
      ...
      /*
      Serial.print("Rejected outlier period: ");
      Serial.print(periodCopy);
      Serial.print(" us | Last good: ");
      Serial.println(lastGoodPeriodUs);
      */
    }
  }

  if (lastAcceptedPulseMs > 0 && millis() - lastAcceptedPulseMs > RPM_ZERO_TIMEOUT_MS) {
    rpm = 0.0;
